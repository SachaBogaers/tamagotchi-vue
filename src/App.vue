	...
	methods: {
		...
		modifyStats(type, amount, name) {
			...
			affectedCows.forEach((cow) => {
				if (type == 'happiness') cow.happiness = Math.max(Math.min(100, cow.happiness + amount))
				else if (type == 'hunger') cow.hunger = Math.max(Math.min(100, cow.hunger + amount))
